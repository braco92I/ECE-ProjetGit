...
class Reseau
{
    ...
    public:
        ...
        nbFrequences=1;
        int freqmax=0;
        m_stations[i]->setFrequence(nbFrequences);
        do
        {
             if (nbFrequences > freqmax)
            {
                freqmax=nbFrequences;
            }
            nbFrequences=1;
            i++;
            do
            {
                if(!m_stations[i]->testFrequence(nbFrequences))
                    nbFrequences++;
                else
                    m_stations[i]->setFrequence(nbFrequences);
            }
            while(m_stations[i]->getFrequence()==0);
        }while(i < m_stations.size()-1);
        nbFrequences=freqmax;
        }
